...
class NFTService {
  private readonly CO2E_CHAIN_RPC = "https://exp.co2e.cc/api/v2";
    ...
    try {
      ...
      if (tokenInfo) {
        result.metadata = {
          ...
          description:
            tokenInfo.description || "Verified carbon credit certificate",
          image: tokenInfo.image || this.generateFallbackImage(contractAddress),
        };
        result.imageUrl =
          tokenInfo.image || this.generateFallbackImage(contractAddress);
      } else {
      ...
      console.error(
        `Error fetching NFT metadata for ${contractAddress}:`,
        error
      );
  ...
  private async fetchTokenInfoFromAPI(contractAddress: string): Promise<any> {
    try {
      ...
      console.warn(
        `Could not fetch token info from API for ${contractAddress}`
      );
      return null;
    }
  }
  ...
  async fetchMultipleNFTMetadata(
    contracts: string[]
  ): Promise<Map<string, NFTContractInfo>> {
